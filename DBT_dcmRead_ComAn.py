# -*- coding: utf-8 -*-
"""
Created on Tue Jan 17 01:29:08 2023
...
def dcmread_image(
    ...
    # print(ds.fileMeta.TransferSyntaxUID)
    ...
    # pixel_array_norm = rescale_intensity(
    #     pixel_array, in_range=(low, high), out_range="dtype"
    # )
    pixel_array_norm = (pixel_array - np.mean(pixel_array)) / np.std(pixel_array)
    return pixel_array, pixel_array_norm
...
# def binary_mask_gen(image):
#     tresh = skimage.filters.threshold_otsu(image)
from pydicom.uid import RLELossless    
def dicom_wrte(image, pth_dcmwrt): # , pth_dcm_im):
    image = image.astype(np.uint16)
    # ds = dicom.dcmread(pth_dcm_im)
    # ds.decompress(handler_name="pylibjpeg")
    # ds.PixelData = image
    ...
    ds.Columns = image.shape[2]
    ds.NumberOfFrames = image.shape[0]
    ...
    # save
    ds.save_as(pth_dcmwrt, write_like_original=False)    
...
for i in range( len(Unqs)):
    ...
    try:
        ...
        Img, Img_norm = dcmread_image(ptt, view)
        
        pt_dcm_im = ptt
        
        Sz = (Img.shape[0],Img.shape[1], Img.shape[2])
        bnrymask = np.ones(Sz , dtype=int)
        
        for j in range(len(indices)):
            min_row = PATHs['X'][indices[j]]
            min_col = PATHs['Y'][indices[j]]
            
            max_row = min_row + PATHs['Width'][indices[j]]
            max_col = min_col + PATHs['Height'][indices[j]]
            
            min_slc = max([0, PATHs['Slice'][indices[j]] - int(0.3*PATHs['VolumeSlices'][indices[j]])])
            max_slc = min([Img.shape[0]-1, PATHs['Slice'][indices[j]] + int(0.3*PATHs['VolumeSlices'][indices[j]])])
            
            croped_img_msk = Img[min_slc:max_slc, min_col:max_col, min_row:max_row]
            # thresh = skimage.filters.threshold_otsu(croped_img)
            # croped_img_msk = croped_img > thresh
            
            bnrymask[min_slc:max_slc, min_col:max_col, min_row:max_row] = croped_img_msk
        
        if not (Img.shape == bnrymask.shape):
            print('!!!')
            bnrymask = np.transpose(bnrymask, axes=[bnrymask.shape.index(Img.shape[0]), 
                                                    bnrymask.shape.index(Img.shape[1]), 
                                                    bnrymask.shape.index(Img.shape[2])])
        pt_dcmwrt = Path(pt.replace('.dcm', '_ComAn_mask.dcm' ))
        dicom_wrte(bnrymask, pt_dcmwrt)
        
        # croped_img = Img
        # thresh = skimage.filters.threshold_otsu(croped_img)
        # croped_img_msk = croped_img > thresh
        
        # Breast_bnrymask = croped_img_msk
        
        # if not (Img.shape == Breast_bnrymask.shape):
        #     print('!!!')
        #     Breast_bnrymask = np.transpose(Breast_bnrymask, axes=[Breast_bnrymask.shape.index(Img.shape[0]), 
        #                                             Breast_bnrymask.shape.index(Img.shape[1]), 
        #                                             Breast_bnrymask.shape.index(Img.shape[2])])
        
        # # Img_norm = (Img - np.mean(Img[Breast_bnrymask]))/np.std(Img[Breast_bnrymask])
        
        # pt_dcmwrt = Path(pt.replace('.dcm', '_crrct.dcm' ))
        # dicom_wrte(Img, pt_dcmwrt)
        
        # pt_dcmwrt = Path(pt.replace('.dcm', '_Breast_mask.dcm' ))
        # dicom_wrte(Breast_bnrymask, pt_dcmwrt)
        ...
        try:
            ...
            print(e)
            print("E:/"+ "Patients with benign cancer" + PATHs['classic_path'][indices[0]][27:])
