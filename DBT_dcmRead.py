# -*- coding: utf-8 -*-
"""
Created on Mon Jul 18 11:27:15 2022
...
for i in range(len(Unqs)):
    ...
    try:
        ...
        pt = "E:/"+ "Patients with benign cancer" + Unqs[i][27:]
        pt = pt.replace('/', '\\' )
        ...
        for j in range(len(indices)):
            ...
            max_slc = min([Img.shape[0]-1, PATHs['Slice'][indices[j]] + int(0.25*PATHs['VolumeSlices'][indices[j]])])
            
            croped_img = Img_norm[min_slc:max_slc, min_col:max_col, min_row:max_row]
        ...
        try:
            indices = PATHs.descriptive_path[PATHs.descriptive_path == Unqs[i]].index.tolist()
            pt = "E:/"+ "Patients with benign cancer" + PATHs['classic_path'][indices[0]][27:]
            pt = pt.replace('/', '\\' )
            pt = pt.replace('0000-', '0000-NA-' )
            ...
            for j in range(len(indices)):
                ...
                min_slc = max([0, PATHs['Slice'][indices[j]] - int(0.25*PATHs['VolumeSlices'][indices[j]])])
                max_slc = min([Img.shape[0]-1, PATHs['Slice'][indices[j]] + int(0.35*PATHs['VolumeSlices'][indices[j]])])
